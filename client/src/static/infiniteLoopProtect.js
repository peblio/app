...
(function (root, factory) {
  ...
  const reSingle = /\b(for|while|do)\b/;
  const labelRe = /\b(?!default:)([a-z_]{1}\w+:)/i;
  const comments = /(?:\/\*(?:[\s\S]*?)\*\/)|(?:([\s;])+\/\/(?:.*)$)/gm;

  const loopProtect = rewriteLoops;
  ...
  function inCommentOrString(index, line) {
    ...
    while (--index > -1) {
      ...
      if (character === '/' || character === '*') {
        ...
        const prevCharacter = line.substr(index - 1, 1);
        if (prevCharacter === '/') {
  ...
  function directlyBeforeLoop(index, lineNum, lines) {
    ...
    const theRest = lines.slice(lineNum).join('\n').substr(index).replace(labelRe, '');
    theRest.replace(reSingle, (match, capture, i) => {
      const target = theRest.substr(0, i).replace(comments, '').trim();
  ...
  function rewriteLoops(code, offset) {
    ...
    const method = `${loopProtect.alias}.protect`;
    const ignore = {};
    const pushonly = {};
    ...
    lines.forEach((line, lineNum) => {
      ...
      const printLineNumber = lineNum - offset + 1;
      let character = '';
        ...
        while (index < line.length) {
          ...
          if (openBrackets === 0 && (character === ';' || character === '{')) {
            ...
            if (character === ';') {
              ...
              const insert = `;if (${method}({ line: ${printLineNumber} })) break;`;
  ...
  loopProtect.protect = function protect(state) {
    ...
    const line = loopProtect.counters[state.line];
